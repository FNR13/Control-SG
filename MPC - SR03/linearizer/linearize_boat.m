...
io(size(io,1)+1) = linio('boat_model/hRoll',1,'output');
io(size(io,1)+1) = linio('boat_model/jZ',1,'output');
...
Bl = [lin_model.B(lateral_model,1) lin_model.B(lateral_model,5)];
Cl = [0 0 0 1];
Dl = 0;
...
save('linear_model.mat','lin_model','LTI_model','initial_uspeed')
