...
plant = setmpcsignals(plant,'MV',[1 2 3],'MD',[4 5]);
...
% Control Weights
W = struct();
W.ManipulatedVariables = [1 1 1];  % Weights for the 3 MVs
W.ManipulatedVariablesRate = [1 1 1];  % Weights for rate of change of the 3 MVs

W.OutputVariables = [1 1];  % Weights for the 2 MOs (roll height)

%% Defining Constraints
% Manipulated variables properties
minAOA = -3; % min angle of attack
maxAOA = 15; % max angle of attack
minAOA_rate = -inf; % min angle of attack rate
maxAOA_rate = inf; % max angle of attack rate

MV(1) = struct('Min', minAOA, 'Max', maxAOA, 'RateMin', minAOA_rate, 'RateMax', maxAOA_rate, 'ScaleFactor', 1);
MV(2) = struct('Min', minAOA, 'Max', maxAOA, 'RateMin', minAOA_rate, 'RateMax', maxAOA_rate, 'ScaleFactor', 1);
MV(3) = struct('Min', minAOA, 'Max', maxAOA, 'RateMin', minAOA_rate, 'RateMax', maxAOA_rate, 'ScaleFactor', 1);
...
% Set IO atributes
setname(mpcobj,'input',1,'Frontfoil right')
setname(mpcobj,'input',2,'Frontfoil left')
setname(mpcobj,'input',3,'Frontfoil rear')
setname(mpcobj,'input',4,'RPMS')
setname(mpcobj,'input',5,'Rudder')
...
initial_MV = [FF_L,FF_R,rear_alfas];
disturbances = [T, gg];
