...
function Rpms_equilibrium = Find_Rpms_Equilibrium(Act_Ailerons, Act_Rear, Rudder, u_0, w_0, p0, q0, r0, Pitch0, Roll0, Yaw0, z_cm)
    ...
    RPMs0 = 1000;
    tolerance = 1e-15;  % Tolerância ajustada
    
    % Opções para otimização
    options = optimset('Display', 'iter', 'TolFun', tolerance, 'TolX', tolerance, 'MaxFunEvals', 5000, 'Algorithm', 'quasi-newton');
    
    % Otimização utilizando fminunc
    Rpms_equilibrium = fminunc(@(Rpms_motor) Equilibrium_Error(Rpms_motor, Act_Ailerons, Act_Rear, Rudder, u_0, w_0, p0, q0, r0, Pitch0, Roll0, Yaw0, z_cm, Target_Forces, Target_Moments), RPMs0, options);
end
